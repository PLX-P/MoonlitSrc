...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
	...
	private final Map<EaglercraftUUID, NetworkPlayerInfo> playerInfoMap = Maps
			.<EaglercraftUUID, NetworkPlayerInfo>newHashMap();
	...
	public NetHandlerPlayClient(Minecraft mcIn, GuiScreen p_i46300_2_, NetworkManager networkManagerIn,
		...
		this.isIntegratedServer = (networkManagerIn instanceof SingleplayerNetworkManager);
	}
	...
	public boolean webViewSendHandler(GameMessagePacket pkt) {
		...
		if (this.gameController.player == null || this.gameController.player.connection != this) {
	...
	public void handleSpawnObject(SPacketSpawnObject packetIn) {
		...
		case 10:
			entity = EntityMinecart.create(this.clientWorldController, d0, d1, d2,
					EntityMinecart.Type.getById(packetIn.getData()));
			break;
		case 90:
			b = true;
		...
		if (packetIn.getType() == 68) {
			entity = new EntityLlamaSpit(this.clientWorldController, d0, d1, d2,
			...
			entity = new EntityItemFrame(this.clientWorldController, new BlockPos(d0, d1, d2),
			...
			entity = new EntityDragonFireball(this.clientWorldController, d0, d1, d2,
					(double) packetIn.getSpeedX() / 8000.0D, (double) packetIn.getSpeedY() / 8000.0D,
					(double) packetIn.getSpeedZ() / 8000.0D);
			b = true;
			...
			entity = new EntityShulkerBullet(this.clientWorldController, d0, d1, d2,
					(double) packetIn.getSpeedX() / 8000.0D, (double) packetIn.getSpeedY() / 8000.0D,
					(double) packetIn.getSpeedZ() / 8000.0D);
			b = true;
			...
			entity = new EntityFallingBlock(this.clientWorldController, d0, d1, d2,
					Block.getStateById(packetIn.getData() & 65535));
			b = true;
	...
	public void handleSpawnPlayer(SPacketSpawnPlayer packetIn) {
		...
		EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(this.gameController.world,
	...
	public void handlePlayerPosLook(SPacketPlayerPosLook packetIn) {
		...
		net.eaglercraft.eaglertone.EaglerToneClientAdapter12.onServerCorrection();
	...
	public void handleMultiBlockChange(SPacketMultiBlockChange packetIn) {
		for (SPacketMultiBlockChange.BlockUpdateData spacketmultiblockchange$blockupdatedata : packetIn
				.getChangedBlocks()) {
			this.clientWorldController.invalidateRegionAndSetBlock(spacketmultiblockchange$blockupdatedata.getPos(),
					spacketmultiblockchange$blockupdatedata.getBlockState());
		}
		SPacketMultiBlockChange.BlockUpdateData[] updates = packetIn.getChangedBlocks();
		if (updates != null && updates.length > 0) {
			BlockPos position = updates[0].getPos();
			net.eaglercraft.eaglertone.EaglerToneClientAdapter12.onChunkChanged(
	...
	public void handleChunkData(SPacketChunkData packetIn) {
		...
		net.eaglercraft.eaglertone.EaglerToneClientAdapter12.onChunkChanged(packetIn.getChunkX(), packetIn.getChunkZ());
		
		List<NBTTagCompound> list = packetIn.getTileEntityTags();		
	...
	public void processChunkUnload(SPacketUnloadChunk packetIn) {
		...
		net.eaglercraft.eaglertone.EaglerToneClientAdapter12.onChunkChanged(packetIn.getX(), packetIn.getZ());
	}
	...
	public void handleBlockChange(SPacketBlockChange packetIn) {
		...
		BlockPos position = packetIn.getBlockPosition();
		net.eaglercraft.eaglertone.EaglerToneClientAdapter12.onBlockChanged(position.getX(), position.getY(), position.getZ());
	}
	...
	public void onDisconnect(ITextComponent reason) {
		Minecraft.getMinecraft().getRenderManager().setEnableFNAWSkins(this.gameController.gameSettings.enableFNAWSkins);
		if (this.gameController.world != null) {
			this.gameController.loadWorld((WorldClient) null);
		}
		if (this.guiScreenServer != null) {
			this.gameController
					.shutdownIntegratedServer(new GuiDisconnected(this.guiScreenServer, "disconnect.lost", reason));
		} else {
	...
	public void handleCollectItem(SPacketCollectItem packetIn) {
		...
		if (entity != null) {
			if (entity instanceof EntityXPOrb) {
				this.clientWorldController.playSound(entity.posX, entity.posY, entity.posZ,
			...
			this.gameController.effectRenderer
					.addEffect(new ParticleItemPickup(this.clientWorldController, entity, entitylivingbase, 0.5F));
	...
	public void handleEntityStatus(SPacketEntityStatus packetIn) {
		...
		if (entity != null) {
			if (packetIn.getOpCode() == 21) {
				...
				this.clientWorldController.playSound(entity.posX, entity.posY, entity.posZ, SoundEvents.ITEM_TOTEM_USE,
	...
	public void handleSetExperience(SPacketSetExperience packetIn) {
		this.gameController.player.setXPStats(packetIn.getExperienceBar(), packetIn.getTotalExperience(),
				packetIn.getLevel());
	}
		...
		if (packetIn.getDimensionID() != this.gameController.player.dimension) {
			...
			this.clientWorldController = new WorldClient(this,
					new WorldSettings(0L, packetIn.getGameType(), false,
							this.gameController.world.getWorldInfo().isHardcoreModeEnabled(), packetIn.getWorldType()),
	...
	public void handleExplosion(SPacketExplosion packetIn) {
		Explosion explosion = new Explosion(this.gameController.world, (Entity) null, packetIn.getX(), packetIn.getY(),
	...
	public void handleOpenWindow(SPacketOpenWindow packetIn) {
		...
		if ("minecraft:container".equals(packetIn.getGuiId())) {
			...
			IInventory iinventory = new ContainerLocalMenu(packetIn.getGuiId(), packetIn.getWindowTitle(),
	...
	public void handleBlockAction(SPacketBlockAction packetIn) {
		this.gameController.world.addBlockEvent(packetIn.getBlockPosition(), packetIn.getBlockType(),
	...
	public void handleBlockBreakAnim(SPacketBlockBreakAnim packetIn) {
		this.gameController.world.sendBlockBreakProgress(packetIn.getBreakerId(), packetIn.getPosition(),
	...
	public void handleChangeGameState(SPacketChangeGameState packetIn) {
		...
		if (i >= 0 && i < SPacketChangeGameState.MESSAGE_NAMES.length
			...
			entityplayer.addChatComponentMessage(
					new TextComponentTranslation(SPacketChangeGameState.MESSAGE_NAMES[i], new Object[0]), false);
		}
		...
		if (i == 1) {
			...
			if (f == 0.0F) {
				...
				this.gameController.ingameGUI.getChatGUI().printChatMessage(new TextComponentTranslation(
						"demo.help.movement",
			...
			this.clientWorldController.playSound(entityplayer, entityplayer.posX,
					entityplayer.posY + (double) entityplayer.getEyeHeight(), entityplayer.posZ,
					SoundEvents.ENTITY_ARROW_HIT_PLAYER, SoundCategory.PLAYERS, 0.18F, 0.45F);
		} else if (i == 7) {
			...
			this.clientWorldController.playSound(entityplayer, entityplayer.posX, entityplayer.posY, entityplayer.posZ,
					SoundEvents.ENTITY_ELDER_GUARDIAN_CURSE, SoundCategory.HOSTILE, 1.0F, 1.0F);
		}
	}
	...
	public void handleEffect(SPacketEffect packetIn) {
		if (packetIn.isSoundServerwide()) {
			this.gameController.world.playBroadcastSound(packetIn.getSoundType(), packetIn.getSoundPos(),
					packetIn.getSoundData());
		} else {
			this.gameController.world.playEvent(packetIn.getSoundType(), packetIn.getSoundPos(),
					packetIn.getSoundData());
		}
	}
	...
	public void handleEntityEffect(SPacketEntityEffect packetIn) {
		...
		if (entity instanceof EntityLivingBase) {
			...
			if (potion != null) {
				PotionEffect potioneffect = new PotionEffect(potion, packetIn.getDuration(), packetIn.getAmplifier(),
	...
	public void handlePlayerListItem(SPacketPlayerListItem packetIn) {
		for (SPacketPlayerListItem.AddPlayerData spacketplayerlistitem$addplayerdata : packetIn.getEntries()) {
			if (packetIn.getAction() == SPacketPlayerListItem.Action.REMOVE_PLAYER) {
				EaglercraftUUID uuid = spacketplayerlistitem$addplayerdata.getProfile().getId();
				...
				NetworkPlayerInfo networkplayerinfo = this.playerInfoMap
						.get(spacketplayerlistitem$addplayerdata.getProfile().getId());
	...
	public void handleSoundEffect(SPacketSoundEffect packetIn) {
		this.gameController.world.playSound(this.gameController.player, packetIn.getX(), packetIn.getY(),
				packetIn.getZ(), packetIn.getSound(), packetIn.getCategory(), packetIn.getVolume(),
				packetIn.getPitch());
	}

	public void handleCustomSound(SPacketCustomSound packetIn) {
		this.gameController.getSoundHandler().playSound(
				new PositionedSoundRecord(new ResourceLocation(packetIn.getSoundName()), packetIn.getCategory(),
						packetIn.getVolume(), packetIn.getPitch(), false, 0, ISound.AttenuationType.LINEAR,
						(float) packetIn.getX(), (float) packetIn.getY(), (float) packetIn.getZ()));
	}
	...
	public void handleResourcePack(SPacketResourcePackSend packetIn) {
		...
		if (this.gameController.getCurrentServerData() != null && this.gameController.getCurrentServerData()
			...
			NetHandlerPlayClient.this.gameController.getResourcePackRepository().downloadResourcePack(s, s1,
					success -> {
						if (success) {
							NetHandlerPlayClient.this.netManager.sendPacket(new CPacketResourcePackStatus(
									CPacketResourcePackStatus.Action.SUCCESSFULLY_LOADED));
						} else {
							NetHandlerPlayClient.this.netManager.sendPacket(
									new CPacketResourcePackStatus(CPacketResourcePackStatus.Action.FAILED_DOWNLOAD));
						}
					});
		...
		if ("MC|TrList".equals(packetIn.getChannelName())) {
			...
			try {
				...
				if (guiscreen != null && guiscreen instanceof GuiMerchant
						&& k == this.gameController.player.openContainer.windowId) {
			...
			ItemStack itemstack = enumhand == EnumHand.OFF_HAND ? this.gameController.player.getHeldItemOffhand()
					: this.gameController.player.getHeldItemMainhand();
			...
			((DebugRendererPathfinding) this.gameController.debugRenderer.debugRendererPathfinding).addPath(l, path,
					f1);
			...
			((DebugRendererNeighborsUpdate) this.gameController.debugRenderer.field_191557_f).func_191553_a(i1,
					blockpos);
	...
	public void handleScoreboardObjective(SPacketScoreboardObjective packetIn) {
		...
		if (packetIn.getAction() == 0) {
			ScoreObjective scoreobjective = scoreboard.addScoreObjective(packetIn.getObjectiveName(),
				...
				for (SPacketEntityProperties.Snapshot spacketentityproperties$snapshot : packetIn.getSnapshots()) {
					...
					if (iattributeinstance == null) {
						iattributeinstance = abstractattributemap.registerAttribute(
								new RangedAttribute((IAttribute) null, spacketentityproperties$snapshot.getName(), 0.0D,
	...
	public void func_194307_a(SPacketPlaceGhostRecipe p_194307_1_) {
		...
		if (container.windowId == p_194307_1_.func_194313_b() && container.getCanCraft(this.gameController.player)) {
			if (this.gameController.currentScreen instanceof IRecipeShownListener) {
				GuiRecipeBook guirecipebook = ((IRecipeShownListener) this.gameController.currentScreen)
						.func_194310_f();
