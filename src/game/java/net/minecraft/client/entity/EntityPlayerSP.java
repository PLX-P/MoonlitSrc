...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdate() {
		if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			...
			if (this.isRiding()) {
				...
				this.connection.sendPacket(new CPacketInput(this.moveStrafing, this.field_191988_bg,
	...
	public void sendChatMessage(String message) {
		if (message != null && message.startsWith("#")) {
			net.eaglercraft.eaglertone.EaglerToneClientAdapter12.executeCommand(message);
			this.mc.ingameGUI.getChatGUI().printChatMessage(new net.minecraft.util.text.TextComponentString(
					net.eaglercraft.eaglertone.EaglerToneClientAdapter12.engine().getLastMessage()));
			return;
		}
	...
	protected void sendHorseJump() {
		this.connection.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.START_RIDING_JUMP,
	...
	public void onLivingUpdate() {
		...
		if (this.inPortal) {
			...
			this.inPortal = false;
		} else if (this.isPotionActive(MobEffects.NAUSEA)) {
			PotionEffect effect = this.getActivePotionEffect(MobEffects.NAUSEA);
		...
		net.eaglercraft.eaglertone.EaglerToneClientAdapter12.applyMovement(this);
		...
		AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
		this.pushOutOfBlocks(this.posX - (double) this.width * 0.35D, axisalignedbb.minY + 0.5D,
	...
	public void updateRidden() {
		...
		if (this.getRidingEntity() instanceof EntityBoat) {
			...
			entityboat.updateInputs(this.movementInput.leftKeyDown, this.movementInput.rightKeyDown,
						...
						if (iblockstate.getCollisionBoundingBox(this.world, blockpos) == null) {
							...
							if (iblockstate1.getCollisionBoundingBox(this.world, blockpos) == null) {
								...
								if (this.isPotionActive(MobEffects.JUMP_BOOST)) {
									f7 += (float) (this.getActivePotionEffect(MobEffects.JUMP_BOOST).getAmplifier() + 1)
											* 0.75F;
								}
								...
								AxisAlignedBB axisalignedbb = (new AxisAlignedBB(vec3d,
										vec3d4.addVector(0.0D, (double) f10, 0.0D)))
										.expand((double) f9, 0.0D, (double) f9);
								...
								for (AxisAlignedBB axisalignedbb2 : list) {
									if (axisalignedbb2.intersects(vec3d7, vec3d8)
										...
										while (true) {
											...
											if ((axisalignedbb1 = iblockstate2.getCollisionBoundingBox(this.world,
													blockpos2)) != null) {
											...
											if (i > 1) {
												...
												if (iblockstate3.getCollisionBoundingBox(this.world,
														blockpos) != null) {
													return;
												}
											}
