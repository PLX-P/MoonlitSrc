...
public abstract class CommandHandler implements ICommandManager {
	...
	/**
	 * Attempt to execute a command. This method should return the number of times
	 * that the command was executed. If the command does not exist or if the player
	 * does not have permission, 0 will be returned. A number greater than 1 can be
	 * returned if a player selector is used.
	 */
		...
		try {
			...
			if (icommand == null) {
				...
				if (j > -1) {
					...
					if (list.isEmpty()) {
						throw new PlayerNotFoundException("commands.generic.selector.notFound",
	...
	protected boolean tryExecute(ICommandSender sender, String[] args, ICommand command, String input) {
		try {
			...
			TextComponentTranslation textcomponenttranslation2 = new TextComponentTranslation("commands.generic.usage",
					new Object[] { new TextComponentTranslation(wrongusageexception.getMessage(),
