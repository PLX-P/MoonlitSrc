...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable {
	...
	public void update() {
		...
		this.playerEntity.setPositionAndRotation(this.firstGoodX, this.firstGoodY, this.firstGoodZ,
		...
		if (this.lowestRiddenEnt != this.playerEntity
				&& this.lowestRiddenEnt.getControllingPassenger() == this.playerEntity) {
			...
			if (this.vehicleFloating
					&& this.playerEntity.getLowestRidingEntity().getControllingPassenger() == this.playerEntity) {
	...
	public void func_194028_b(final ITextComponent p_194028_1_) {
		this.netManager.sendPacket(new SPacketDisconnect(p_194028_1_));
		this.netManager.closeChannel(p_194028_1_);
	}
	
	public void func_194028_b(final String p_194028_1_) {
		final ITextComponent component = new TextComponentString(p_194028_1_);
		this.netManager.sendPacket(new SPacketDisconnect(component));
		this.netManager.closeChannel(component);
	}
	...
	public void processInput(CPacketInput packetIn) {
		this.playerEntity.setEntityActionState(packetIn.getStrafeSpeed(), packetIn.func_192620_b(),
	...
	public void processVehicleMove(CPacketVehicleMove packetIn) {
		if (isMoveVehiclePacketInvalid(packetIn)) {
			...
			if (entity != this.playerEntity && entity.getControllingPassenger() == this.playerEntity
				...
				if (d10 - d9 > 100.0D && (!this.serverController.isSinglePlayer()
					...
					LOGGER.warn("{} (vehicle of {}) moved too quickly! {},{},{}", entity.getName(),
							this.playerEntity.getName(), Double.valueOf(d6), Double.valueOf(d7), Double.valueOf(d8));
					this.netManager.sendPacket(new SPacketMoveVehicle(entity));
					return;
				}

				boolean flag = worldserver.getCollisionBoxes(entity, entity.getEntityBoundingBox().contract(0.0625D))
						.isEmpty();
				...
				boolean flag2 = worldserver.getCollisionBoxes(entity, entity.getEntityBoundingBox().contract(0.0625D))
						.isEmpty();
				...
				this.vehicleFloating = d11 >= -0.03125D && !this.serverController.isFlightAllowed()
						&& !worldserver.checkBlockCollision(
								entity.getEntityBoundingBox().expandXyz(0.0625D).addCoord(0.0D, -0.55D, 0.0D));
			...
			if (!this.playerEntity.playerConqueredTheEnd) {
				...
				if (this.targetPos != null) {
					if (this.networkTickCount - this.lastPositionUpdate > 20) {
						...
						this.setPlayerLocation(this.targetPos.xCoord, this.targetPos.yCoord, this.targetPos.zCoord,
								this.playerEntity.rotationYaw, this.playerEntity.rotationPitch);
					}
				} else {
					this.lastPositionUpdate = this.networkTickCount;
					...
					if (this.playerEntity.isRiding()) {
						this.playerEntity.setPositionAndRotation(this.playerEntity.posX, this.playerEntity.posY,
								this.playerEntity.posZ, packetIn.getYaw(this.playerEntity.rotationYaw),
						...
						double d10 = this.playerEntity.motionX * this.playerEntity.motionX
						...
						if (this.playerEntity.isPlayerSleeping()) {
							...
							if (i > 5) {
								LOGGER.debug("{} is sending move packets too frequently ({} packets since last tick)",
										this.playerEntity.getName(), Integer.valueOf(i));
								i = 1;
							}

							if (!this.playerEntity.isInvulnerableDimensionChange() && (!this.playerEntity
									.getServerWorld().getGameRules().getBoolean("disableElytraMovementCheck")
									|| !this.playerEntity.isElytraFlying())) {
								float f2 = this.playerEntity.isElytraFlying() ? 300.0F : 100.0F;

								if (d11 - d10 > (double) (f2 * (float) i)
										&& (!this.serverController.isSinglePlayer() || !this.serverController
												.getServerOwner().equals(this.playerEntity.getName()))) {
									LOGGER.warn("{} moved too quickly! {},{},{}", this.playerEntity.getName(),
							...
							boolean flag2 = worldserver.getCollisionBoxes(this.playerEntity,
									this.playerEntity.getEntityBoundingBox().contract(0.0625D)).isEmpty();
							...
							if (!this.playerEntity.isInvulnerableDimensionChange() && d11 > 0.0625D
							...
							if (!this.playerEntity.noClip && !this.playerEntity.isPlayerSleeping()) {
								boolean flag1 = worldserver.getCollisionBoxes(this.playerEntity,
										this.playerEntity.getEntityBoundingBox().contract(0.0625D)).isEmpty();
							...
							this.floating &= !this.playerEntity.isPotionActive(MobEffects.LEVITATION)
									&& !this.playerEntity.isElytraFlying()
									&& !worldserver.checkBlockCollision(this.playerEntity.getEntityBoundingBox()
											.expandXyz(0.0625D).addCoord(0.0D, -0.55D, 0.0D));
							this.playerEntity.onGround = packetIn.isOnGround();
							this.serverController.getPlayerList().serverUpdateMovingPlayer(this.playerEntity);
	...
	public void processRightClickBlock(CPacketPlayerTryUseItemOnBlock packetIn) {
		...
		if (blockpos.getY() < this.serverController.getBuildLimit() - 1
			...
			if (this.targetPos == null
					...
					&& worldserver.getWorldBorder().contains(blockpos)) {
				this.playerEntity.interactionManager.processRightClickBlock(this.playerEntity, worldserver, itemstack,
						enumhand, blockpos, enumfacing, packetIn.getFacingX(), packetIn.getFacingY(),
						packetIn.getFacingZ());
			}
		} else {
		...
		if (this.playerEntity.isSpectator()) {
			...
			if (entity != null) {
				...
				if (entity.world == this.playerEntity.world) {
					...
					this.sendPacket(new SPacketRespawn(this.playerEntity.dimension, worldserver1.getDifficulty(),
					...
					this.serverController.getPlayerList().updateTimeAndWeatherForPlayer(this.playerEntity,
							worldserver2);
	...
	public void sendPacket(final Packet<?> packetIn) {
		if (packetIn instanceof SPacketChat) {
			...
			if (entityplayer$enumchatvisibility == EntityPlayer.EnumChatVisibility.HIDDEN
					&& spacketchat.func_192590_c() != ChatType.GAME_INFO) {
				return;
			}
	...
	public void processChatMessage(CPacketChatMessage packetIn) {
		if (this.playerEntity.getChatVisibility() == EntityPlayer.EnumChatVisibility.HIDDEN) {
			...
			if (this.chatSpamThresholdCount > 200
					&& !this.serverController.getPlayerList().canSendCommands(this.playerEntity.getGameProfile())) {
	...
	public void processEntityAction(CPacketEntityAction packetIn) {
		...
		case START_FALL_FLYING:
			if (!this.playerEntity.onGround && this.playerEntity.motionY < 0.0D && !this.playerEntity.isElytraFlying()
					&& !this.playerEntity.isInWater()) {
			...
			if (this.playerEntity.getDistanceSqToEntity(entity) < d0) {
				if (packetIn.getAction() == CPacketUseEntity.Action.INTERACT) {
					...
					if (entity instanceof EntityItem || entity instanceof EntityXPOrb || entity instanceof EntityArrow
						...
						this.func_194028_b(new TextComponentTranslation(
	...
	public void processClientStatus(CPacketClientStatus packetIn) {
		...
		case PERFORM_RESPAWN:
			if (this.playerEntity.playerConqueredTheEnd) {
				...
				this.playerEntity = this.serverController.getPlayerList().recreatePlayerEntity(this.playerEntity, 0,
						true);
				CriteriaTriggers.field_193134_u.func_193143_a(this.playerEntity, DimensionType.THE_END,
				...
				if (this.serverController.isHardcore()) {
					...
					this.playerEntity.getServerWorld().getGameRules().setOrCreateGameRule("spectatorsGenerateChunks",
							"false");
				}
			}
	...
	public void processClickWindow(CPacketClickWindow packetIn) {
		...
		if (this.playerEntity.openContainer.windowId == packetIn.getWindowId()
			...
			if (this.playerEntity.isSpectator()) {
				...
				ItemStack itemstack2 = this.playerEntity.openContainer.slotClick(packetIn.getSlotId(),
						packetIn.getUsedButton(), packetIn.getClickType(), this.playerEntity);
				...
				if (ItemStack.areItemStacksEqual(packetIn.getClickedItem(), itemstack2)) {
					...
					this.pendingTransactions.addKey(this.playerEntity.openContainer.windowId,
	...
	public void func_194308_a(CPacketPlaceRecipe p_194308_1_) {
		...
		if (!this.playerEntity.isSpectator() && this.playerEntity.openContainer.windowId == p_194308_1_.func_194318_a()
				&& this.playerEntity.openContainer.getCanCraft(this.playerEntity)) {
			this.field_194309_H.func_194327_a(this.playerEntity, p_194308_1_.func_194317_b(),
					p_194308_1_.func_194319_c());
		}
	}
	...
	public void processCreativeInventoryAction(CPacketCreativeInventoryAction packetIn) {
		if (this.playerEntity.interactionManager.isCreative()) {
			...
			if (!itemstack.func_190926_b() && itemstack.hasTagCompound()
			...
			boolean flag2 = itemstack.func_190926_b()
					|| itemstack.getMetadata() >= 0 && itemstack.func_190916_E() <= 64 && !itemstack.func_190926_b();
	...
	public void processTabComplete(CPacketTabComplete packetIn) {
		...
		this.playerEntity.connection
				.sendPacket(new SPacketTabComplete((String[]) list.toArray(new String[list.size()])));
	}
	...
	public void processCustomPayload(CPacketCustomPayload packetIn) {
		...
		if ("MC|BEdit".equals(s)) {
			...
			try {
				...
				if (l == 0) {
					TileEntity tileentity = this.playerEntity.world.getTileEntity(
			...
			try {
				...
				TileEntityCommandBlock.Mode tileentitycommandblock$mode = TileEntityCommandBlock.Mode
						.valueOf(packetbuffer3.readStringFromBuffer(16));
				boolean flag = packetbuffer3.readBoolean();
				boolean flag1 = packetbuffer3.readBoolean();

				if (commandblockbaselogic != null) {
					EnumFacing enumfacing = (EnumFacing) this.playerEntity.world.getBlockState(blockpos1)
					...
					case SEQUENCE:
						...
						this.playerEntity.world.setBlockState(blockpos1,
				...
				if (tileentity1 instanceof TileEntityStructure) {
					...
					if (l1 == 2) {
						if (tileentitystructure.save()) {
							this.playerEntity.addChatComponentMessage(
			...
			try {
				...
				this.playerEntity.connection.sendPacket(new SPacketSetSlot(-2, this.playerEntity.inventory.currentItem,
						this.playerEntity.inventory.getStackInSlot(this.playerEntity.inventory.currentItem)));
				this.playerEntity.connection
						.sendPacket(new SPacketSetSlot(-2, j1, this.playerEntity.inventory.getStackInSlot(j1)));
				this.playerEntity.connection
						.sendPacket(new SPacketHeldItemChange(this.playerEntity.inventory.currentItem));
			} catch (Exception exception) {
				LOGGER.error("Couldn't pick item", (Throwable) exception);
			}
		} else if ("EAG|MyUpdCert-1.8".equals(s)) {
			//do nothing
		} else {
			try {
				eaglerMessageController.handlePacket(packetIn.getChannelName(), packetIn.getBufferData());
			} catch (IOException e) {
				LOGGER.error("Couldn't read \"{}\" packet as an eaglercraft plugin message!", packetIn.getChannelName());
				LOGGER.error(e);
			}
		}
	}
